...
typedef NS_ENUM(int, ContentLabelFontSize) {
    ...
    SmallSizeForContentLabel       = 14,
    NormalSizeForContentLabel      = 17,
    BigSizeForContentLabel         = 20
...
+ (instancetype)setTipsContentWithContent:(NSString *)content calcelAction:(cancelBlock)cancelBlock okAction:(okBlock)okBlock {
    TipsView * tip = [[TipsView alloc] initWithFrame:CGRectMake(0, 0, 240, 98)];
    [tip showTips:content cancelTarget:cancelBlock okTarget:okBlock];
    
    // 设置阴影
//    tip.layer.shadowColor = [UIColor redColor].CGColor;
//    tip.layer.shadowOffset = CGSizeMake(5, 5);
//    [tip.layer setShadowColor:[UIColor blackColor].CGColor];
//    [tip.layer setShadowOffset:CGSizeMake(5, 5)];
//    [tip.layer setShadowRadius:5];
//    [tip.layer setShadowPath:[UIBezierPath bezierPathWithRect:tip.bounds].CGPath];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    ...
    newRect.size = CGSizeMake(exObject_ScreenFrame.size.width/4*3, self.confirmBtn.frame.origin.y + self.confirmBtn.frame.size.height + 5);
    
    CGRect cancelOfOrigin = self.cancelBtn.frame;
    CGRect okOfOrigin = self.confirmBtn.frame;
    
    cancelOfOrigin.origin.y = self.contentLabel.frame.origin.y + self.contentLabel.frame.size.height + 5;
    okOfOrigin.origin.y = self.contentLabel.frame.origin.y + self.contentLabel.frame.size.height + 5;
...
- (void)setBlurBackgounrd {
    ...
    effectView.frame = windowView.frame;
    effectView.alpha = 0;
...
- (void)resetBlurBackgournd {
    ...
    for (UIView * effect in windowView.subviews) {
